...
export const AuthorizeUserRoute: React.FC<ProtectUserRouteProps> = ({
    children,
}) => {
    const router = useRouter();
    const accessToken = useTokenStorage.getState().accessToken;
    ...
    // Thay đổi state để kiểm tra người dùng có hợp lệ hay không
    const [isAuthorized, setIsAuthorized] = useState<boolean | null>(null);
    const setCurrentUserID = useCurrentUserStore((state) => state.updateId);
    useEffect(() => {
        if (!accessToken) {
            // Nếu không coó accessToken thì đẩy về login
            router.push("/login");
            setIsAuthorized(false);
        } else {
            try {
